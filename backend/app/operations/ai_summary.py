...
unique counts, min/max/mean/sum -- never raw cell rows) and asks the
configured AIProvider for a plain-English summary. Data minimization by
construction: the AI never sees actual cell values, only aggregate shape,
per the project's AI architecture policy.
"""
...
def _build_prompt(sheet_name: str, columns: list[ColumnAnalysis]) -> str:
    column_lines = "\n".join(_format_column(c) for c in columns)
    return (
        "You are summarizing a spreadsheet for a non-technical user (an accountant, "
        "student, or freelancer). You are given only column-level statistics, not the "
        "actual data. Write a short (3-5 sentence) plain-English summary of what this "
        "sheet likely contains and anything notable (missing data, mixed types, etc). "
        "Do not invent specific values you weren't given.\n\n"
        f"Sheet: {sheet_name}\n"
        f"Columns:\n{column_lines}\n"
    )
...
async def summarize_workbook(
    ...
    prompt = _build_prompt(sheet.name, sheet.columns)
